...
# stagger feet front/back at startup
set_q("right_hip",   22)
set_q("left_hip",   -22)
set_q("right_knee", -12)
set_q("left_knee",  -12)
...
def get_pose(stance, swing):
    ...
    ts["position"][0] += T_PX
    return p_sw, ts, p_st_w, R_st_w


def compute_dq(v_stance, v_swing, stance, R_st_w, p_st_w):
    """
    convert foot velocities from stance frame into joint velocities.
    fix: use actual p_st_w (not zeros) so torso frame is correct.
    """
    q      = get_q_vec()
    ...
    # get torso orientation in stance frame using the real foot position
    ts = utils.torso_state_in_stance_frame(
        model, data, p_c=p_st_w, R_c=R_st_w, torso_name="torso")
    R = ts["orientation"]

    # rotate velocities from stance frame to torso frame
    vs = R.T @ v_stance.reshape(3, 1)
    vw = R.T @ v_swing.reshape(3, 1)

    # 2D jacobian: only x and z components
...
class Rocker:
    def __init__(self, t0):
        ...
        self.min_dt       = 0.3
        p_sw, _, _, _ = get_pose(self.stance, self.swing)
    ...
    def step(self, contact_l, contact_r, t):
        ...
        p_sw, ts, p_st_w, R_st_w = get_pose(self.stance, self.swing)

        # swing foot target — fixed positions, alternates each step
        # right stance: left foot goes behind at -STEP_LENGTH
        # left stance:  right foot goes forward at +STEP_LENGTH
        ...
        # fix: pass p_st_w so the frame rotation is correct
...
# warm-up: stabilize before viewer opens
print("stabilizing...")
